 ...
 * @enum {string}
 */
TypeManager.Visibility = {
  PRIVATE: 'private',
  PROTECTED: 'protected',
  PUBLIC: 'public'
};
var Visibility = TypeManager.Visibility;
...
TypeManager.prototype.defType = function(name, reason, visibility, innerType) {
  ...
  // type definition file and the reason is sufficient.
  if (this.fileFinder_ && !infer.cx().definitions[base.origin] &&
      this.shouldLoad_(reason, visibility)) {
...
/**
 * Whether we should attempt to find and load the file providing a type which we
 * are defining for the given reason.
 * @param {Reason} reason
 * @param {Visibility} visibility
 * @return {boolean}
 * @private
 */
TypeManager.prototype.shouldLoad_ = function(reason, visibility) {
  if (!this.ternServer_.findFile(infer.cx().curOrigin).parent) {
    // We are currently processing a file that has been added directly (not
    // automatically loaded because it was referenced from another file), so
    // types should be loaded regardless of the reason or visibility.
    return true;
  }
  // Otherwise, we only load types that could be exposed to an including file.
